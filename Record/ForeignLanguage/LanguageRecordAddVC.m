...
@interface LanguageRecordAddVC ()<LanguageItemDelegate,UITextFieldDelegate>
...
@implementation LanguageRecordAddVC
...
#pragma mark - 初始化页面
- (void)initPage{
    self.navigationItem.title = @"添加外语学习记录";
    ...
    _lblType = [UITools createLabelWithFrame:CGRectMake(lblTypeTitle.right, lblTypeTitle.top, vBg.width - lblTypeTitle.width - 20, lblTypeTitle.height)
                               ...
                               textAlignment:NSTextAlignmentRight
                                   SuperView:vBg];
    if (!self.languageRecord && !self.languageType) {
        __weak LanguageRecordAddVC *weakSelf = self;
    ...
    _lblTime = [UITools createLabelWithFrame:CGRectMake(lblTimeTitle.right, lblTimeTitle.top, vBg.width - lblTimeTitle.width - 20, lblTimeTitle.height)
                               ...
                               textAlignment:NSTextAlignmentRight
                                   SuperView:vBg];
    if (!self.languageRecord) {
        __weak LanguageRecordAddVC *weakSelf1 = self;
    ...
    if (self.languageRecord) {
        self.lblType.text = [NSString stringWithFormat:@"%@ >",self.languageRecord.language_name];
        self.tfCount.text = [NSString stringWithFormat:@"%i",self.languageRecord.language_count];
    }else if (self.languageType) {
        self.lblType.text = [NSString stringWithFormat:@"%@ >",self.languageType.name];
    }
...
- (void)initData{
    if (self.languageRecord) {
        self.strSportsTime = self.languageRecord.language_time;
    }else{
...
- (void)clickBtnAdd{
    ...
    if (!self.languageType && !self.languageRecord) {
        [self showMessage:@"请选择外语项目"];
        return;
    }
    if (!self.strSportsTime || [self.strSportsTime stringByTrim].length == 0) {
        [self showMessage:@"请选择学习日期"];
        return;
    }
    ...
    NSString *strKeyword = @"";
    if (self.languageRecord) {
        strKeyword = self.languageRecord.language_keyword;
    }else{
        strKeyword = self.languageType.keyword;
    }
    BOOL flag = [Language addObjectWithTime:self.strSportsTime andKeyword:strKeyword andCount:[self.tfCount.text intValue]];
    if (flag) {
        [self showMessage:@"保存学习记录成功"];
        [self performSelector:@selector(popVC) withObject:nil afterDelay:1.5];
    }else{
        [self showMessage:@"保存学习记录失败"];
    }
}
...
- (void)selectLanguageItem:(LanguageItem *)languageItem{
    self.languageType = languageItem;
    self.lblType.text = [NSString stringWithFormat:@"%@ >",languageItem.name];
}
