...
@interface LanguageItemListVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation LanguageItemListVC
...
- (void)initRightNavigationBarButtonItem{
    UIBarButtonItem *rightBtnItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addLanguageItem)];
...
- (void)refreashData{
    ...
    [self.arrSource addObjectsFromArray:[LanguageItem searchLanguageItemWithKeyword:@""]];
    [self.tbvMain reloadData];
}

#pragma mark - 按钮点击事件
- (void)addLanguageItem{
    LanguageItemAddVC *vc = [LanguageItemAddVC new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.arrSource.count) {
        LanguageItem *item = [self.arrSource objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"你确定删除该外语项目？" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(selectLanguageItem:)]) {
        if (indexPath.row < self.arrSource.count) {
            LanguageItem *item = [self.arrSource objectAtIndex:indexPath.row];
            [self.delegate selectLanguageItem:item];
            [self popVC];
        }
    }
}
...
- (void)deleteItemAtIndex:(NSInteger)index{
    if (index < self.arrSource.count) {
        LanguageItem *item = [self.arrSource objectAtIndex:index];
        BOOL flag = [LanguageItem deleteObjectKeyword:item.keyword];
        if (flag) {
            [self.arrSource removeObject:item];
            [self.tbvMain reloadData];
        }else{
            [self showMessage:@"删除外语项目失败"];
        }
    }
}
@end
