...
@implementation ForeignLanguageVC
...
- (void)generateStatisticsPage{
    ...
    for (int i = 0; i < self.arrData.count; i++) {
        LanguageItem *item = [self.arrData objectAtIndex:i];
        NSArray *arrRecord = [Language searchLanguageWithKeyword:item.keyword andTime:self.strSearchTime];
        int64_t countAll = 0;
        for (Language *sportRecord in arrRecord) {
            countAll += sportRecord.language_count;
        }
...
- (void)fetchData{
    ...
    [self.arrData addObjectsFromArray:[LanguageItem searchLanguageItemWithKeyword:nil]];
